...
@dag(
    ...
    catchup=False,
    tags=[f'Loading raw sport formula1 data into Airflow'],
)

def el_sport_formula1_data_into_mongodb():
    @task
    ...
    all_articles = extract_from_newsapi("Formula 1", datetime.today().date() - timedelta(days=1), datetime.today().date(),
                                        "en", 1)
    load_raw_data(all_articles)

el_sport_formula1_data_into_mongodb()