...
@dag(
    ...
    catchup=False,
    tags=[f'Loading raw sport boxing data into Airflow'],
)

def el_sport_boxing_data_into_mongodb():
    @task
    ...
    all_articles = extract_from_newsapi("boxing", datetime.today().date() - timedelta(days=1), datetime.today().date(),
                                        "en", 1)
    load_raw_data(all_articles)

el_sport_boxing_data_into_mongodb()