...
@dag(
    ...
    catchup=False,
    tags=[f'Loading raw sport shiba_inu data into Airflow'],
)

def el_crypto_shiba_inu_data_into_mongodb():
    @task
    ...
    all_articles = extract_from_newsapi("Shiba Inu", datetime.today().date() - timedelta(days=1), datetime.today().date(),
                                        "en", 1)
    load_raw_data(all_articles)

el_crypto_shiba_inu_data_into_mongodb()