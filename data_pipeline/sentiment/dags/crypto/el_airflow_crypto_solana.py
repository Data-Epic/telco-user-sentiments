...
@dag(
    ...
    catchup=False,
    tags=[f'Loading raw sport solana data into Airflow'],
)

def el_crypto_solana_data_into_mongodb():
    @task
    ...
    all_articles = extract_from_newsapi("Solana", datetime.today().date() - timedelta(days=1), datetime.today().date(),
                                        "en", 1)
    load_raw_data(all_articles)

el_crypto_solana_data_into_mongodb()