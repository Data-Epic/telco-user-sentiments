...
@dag(
    ...
    catchup=False,
    tags=[f'Loading raw sport nissan_leaf data into Airflow'],
)

def el_electric_nissan_leaf_data_into_mongodb():
    @task
    ...
    all_articles = extract_from_newsapi("Nissan Leaf", datetime.today().date() - timedelta(days=1), datetime.today().date(),
                                        "en", 1)
    load_raw_data(all_articles)

el_electric_nissan_leaf_data_into_mongodb()