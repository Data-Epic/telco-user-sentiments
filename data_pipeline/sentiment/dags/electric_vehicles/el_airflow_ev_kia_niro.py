...
@dag(
    ...
    catchup=False,
    tags=[f'Loading raw sport kia_niro data into Airflow'],
)

def el_electric_kia_niro_data_into_mongodb():
    @task
    ...
    all_articles = extract_from_newsapi("Kia Niro", datetime.today().date() - timedelta(days=1), datetime.today().date(),
                                        "en", 1)
    load_raw_data(all_articles)

el_electric_kia_niro_data_into_mongodb()