...
@dag(
    ...
    catchup=False,
    tags=[f'Loading raw sport audi_etron data into Airflow'],
)

def el_electric_audi_etron_data_into_mongodb():
    @task
    ...
    all_articles = extract_from_newsapi("Audi e-tron", datetime.today().date() - timedelta(days=1), datetime.today().date(),
                                        "en", 1)
    load_raw_data(all_articles)

el_electric_audi_etron_data_into_mongodb()