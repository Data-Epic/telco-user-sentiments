...
@dag(
    ...
    catchup=False,
    tags=[f'Loading raw sport chevrolet_bolt data into Airflow'],
)

def el_electric_chevrolet_bolt_data_into_mongodb():
    @task
    ...
    all_articles = extract_from_newsapi("Chevrolet Bolt", datetime.today().date() - timedelta(days=1), datetime.today().date(),
                                        "en", 1)
    load_raw_data(all_articles)

el_electric_chevrolet_bolt_data_into_mongodb()