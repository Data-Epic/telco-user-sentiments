...
@dag(
    ...
    catchup=False,
    tags=[f'Loading raw sport lucid_air data into Airflow'],
)

def el_electric_lucid_air_data_into_mongodb():
    @task
    ...
    all_articles = extract_from_newsapi("Lucid Air", datetime.today().date() - timedelta(days=1), datetime.today().date(),
                                        "en", 1)
    load_raw_data(all_articles)

el_electric_lucid_air_data_into_mongodb()