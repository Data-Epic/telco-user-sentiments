...
@dag(
    ...
    catchup=False,
    tags=[f'Loading raw sport bmwi3 data into Airflow'],
)

def el_electric_bmwi3_data_into_mongodb():
    @task
    ...
    all_articles = extract_from_newsapi("BMW i3", datetime.today().date() - timedelta(days=1), datetime.today().date(),
                                        "en", 1)
    load_raw_data(all_articles)

el_electric_bmwi3_data_into_mongodb()